...
def login():
    if request.method == "POST":
        ...
        with get_db_connection() as conn:
            usuario = conn.execute("SELECT id, verificado FROM usuarios WHERE email = ? AND senha = ?", (email, senha)).fetchone()
...
def registro():
    if request.method == "POST":
        ...
        with get_db_connection() as conn:
            try:
                conn.execute("INSERT INTO usuarios (email, senha, codigo_verificacao) VALUES (?, ?, ?)", (email, senha, codigo_verificacao))
                conn.commit()
